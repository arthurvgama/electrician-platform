 ...
 * Supabase (see js-auth.js and supabase-config.js); this file never touches
 * localStorage except for the one-time import of pre-login data on first login.
 *
...
// persisted to its own localStorage key so a reload restores exactly this shape.
let appState = {
  requests: [],
  settings: {
    ...
    // AI provider config (any provider supported via AIProvider). Stored per-browser.
    aiProvider: '',
    aiApiKey: '',
    aiModel: '',
    aiBaseUrl: '',
  ...
  // Purchase list state, persisted under 'electrician_purchase'. qty/inStock/units are
...
// Find a job by id, tolerating string/number id mismatches (older saved data used
// numeric ids; newer imports use string ids, and inline onclick passes ids as strings).
function findJob(id) {
...
// a network round trip. Populated by initializeAppData(), cleared on sign-out.
...
// If this is a brand-new account (nothing saved yet) and this browser still has
// pre-login localStorage data, offer a one-time import so existing jobs aren't
// lost in the move to per-account storage. Mutates `row` in place on import.
async function maybeOfferLocalImport(row) {
  const looksEmpty = (!Array.isArray(row.requests) || row.requests.length === 0)
    && (!row.settings || Object.keys(row.settings).length === 0)
    && (!row.purchase || Object.keys(row.purchase).length === 0);
  if (!looksEmpty) return;

  const localRequests = localStorage.getItem('electrician_requests');
  const localSettings = localStorage.getItem('electrician_settings');
  const localPurchase = localStorage.getItem('electrician_purchase');
  if (!localRequests && !localSettings && !localPurchase) return;

  const importIt = confirm('This browser has service request data saved from before you logged in. Import it into your new account now? This only happens once.');
  if (!importIt) return;

  try {
    if (localRequests) row.requests = JSON.parse(localRequests);
    if (localSettings) row.settings = JSON.parse(localSettings);
    if (localPurchase) row.purchase = JSON.parse(localPurchase);
  } catch (err) {
    console.error('Could not parse local data for import', err);
    return;
  }

  const { error } = await Auth.client.from('user_data')
    .update({ requests: row.requests, settings: row.settings, purchase: row.purchase })
    .eq('user_id', currentUserId);
  if (error) console.error('Failed to save imported data to your account', error);
}
...
window.App.onSignedIn = async function (user) {
  ...
  await maybeOfferLocalImport(_userRow);
  await loadSettings();
  await loadData();
  await loadPurchase();
...
// ============ SETTINGS MANAGEMENT ============
// Reads from the cached row loaded by initializeAppData() (see window.App.onSignedIn);
// no network round trip on its own.
async function loadSettings() {
  ...
  // Migrate an older Groq-only key onto the unified provider config.
  if (appState.settings.groqApiKey && !appState.settings.aiApiKey) {
    appState.settings.aiProvider = 'groq';
    appState.settings.aiApiKey = appState.settings.groqApiKey;
  }
  delete appState.settings.groqApiKey;
  updateBaseLabel();
}
...
// provider config to appState + localStorage. Invalid hour/cap combinations are rejected
...
// callers), matching the old synchronous localStorage write's call sites.
...
// Fire-and-forget (not awaited by callers), matching the old synchronous localStorage write.
...
// (not awaited by callers), matching the old synchronous localStorage write.
