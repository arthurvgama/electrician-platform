...
const InferenceEngine = (function () {
  ...
  // another letter/digit, so "spa" no longer matches "space", "smoke" no longer
  // matches "smoker", "fix" no longer matches "fixture", "plug" no longer matches
  // "unplugged", "urgent" no longer matches inside a bigger word, etc. Because of
  // this, stem keywords must list their inflected forms explicitly (see keyword
  // lists below: e.g. trip/trips/tripped/tripping). Compiled regexes are cached.
  const _wordRe = {};
  ...
  // NOTE (v3): with whole-word matching, stems must list their inflections here
  // (e.g. "melt" no longer covers "melted"/"melting" by substring).
  const HAZARD_WORDS = [
  ...
  // Inflections listed explicitly (v3 whole-word matching): "break" no longer covers
  ...
  // Turn the raw keyword scores into a coarse confidence signal the app can use to
  // decide whether to fall back to an AI provider. This measures how UNAMBIGUOUS the
  ...
  //   'outofscope' - a new install/project (offline handles these without the API)
